...
직접 계산하도록 수정했고, 수정 후 가족 rsid 합집합이 정확히 1,117,586개로 재현됨을 확인해
로직이 동일함을 검증했다(전체 900만 행 ClinVar 스트리밍 재매칭 자체는 실행 시간이 길어
재구성 작업에서는 재실행하지 않았고, 기존에 커밋돼 있던 `family_clinvar_matches.csv` 결과를
그대로 유지했다).
